...
def save_pred_and_gt_json(total_eval_rt_list, total_instance_list, total_gt_rt_list, gt_cls_list, total_rt_list,save_path):


    rt = np.stack(total_eval_rt_list) 
    gt = np.stack(total_gt_rt_list)
    cls = np.stack(gt_cls_list) 

    change_rt = np.stack(total_rt_list) 
    ...
    for instance_pred in rt.tolist():  
        ...
        eval_r_ls.append(pred_R)
        eval_t_ls.append(pred_T)


    for instance_pred in gt.tolist():  
        ...
        gt_r_ls.append(pred_R)
        gt_t_ls.append(pred_T)



    for instance_pred in change_rt.tolist():  
        ...
        change_r_ls.append(pred_R)
        change_t_ls.append(pred_T)


    # save_csv
    dataframe = pd.DataFrame({'scene_id': scene_id_ls, 'im_id': im_id_ls, 'obj_id': obj_id_ls, 'score': score_ls,
                            'eval_R': eval_r_ls, 'eval_t': eval_t_ls, 'time': time_ls,'gt_R':gt_r_ls,'gt_t':gt_t_ls,
                            'change_R':change_r_ls,'change_t': change_t_ls})
