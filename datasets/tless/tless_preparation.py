...
def get_data(trian_root_path, gt_mask):
    ...
    for dir_name in files:
        ...
        for instance_mask in visiblemask:
            ...
            if 'test' in rgb_dir:
                instance_rgb_path = rgb_dir + instance_mask[:6] + '.png' 

            else:
                instance_rgb_path = rgb_dir + instance_mask[:6] + '.jpg' 
    ...
    print("data done : {}".format(len(data_package)) )
...
def prepare_data(input_data):
    ...
    obj_id = instance_gt_info['obj_id']
    ...
    if len(mask.nonzero()[0]) >= mask_thershold:
        ...
        print('instance save direction:', instance_save_dir[:-13])
...
def translate_gt_mask2_pred_mask(data_path):
    ...
    for gt_mask in gt_mask_path:
        ...
        instance_gt_mask = np.array(Image.open(gt_mask)) 
...
if __name__ == '__main__':
    ...
    if not opt.train_set:
        ...
        f.close()
        print("the gt mask processing is done!")
        ...
        # # prepare train set
        print("start the train prepared!")
        ...
        print("the length of the data_package:", len(data_package))
        ...
        f.close()
        print("the training dataset peocessing is done ")
